...
namespace Fuzzy.Components
{
	...
	public class FluentRenderTreeBuilder: IDisposable
	{
			...
			public FrameInfo (FrameType frameType, string? name = null)
			{
				if (frameType == FrameType.Element && string.IsNullOrEmpty (name))
					throw new ArgumentNullException (nameof (name),
							"For element frames, a name must be specified");
		...
		readonly Stack<FrameInfo> _frames = new();
...
#pragma warning disable BL0006 // Do not use RenderTree types
		...
		/// <summary>
		/// Calls <see cref="RenderTreeBuilder.AddAttribute(int, RenderTreeFrame)">AddAttribute(...)</see>
				...
				[CallerLineNumber] int line = 0)
		{
			if (action?.Target == null)
				throw new ArgumentNullException (nameof (action),
						$"'{nameof (action)}' or '{nameof (action.Target)}' is null");
		...
		/// <summary>
		/// Calls <see cref="Ref{T}(Action{T}, int)">Ref(...)</see>
		...
		/// <param name="line">The source code line number used to generate the sequence number.</param>
		[Obsolete ("Renamed, and will be removed in the future; use Ref<T> instead")]
		public FluentRenderTreeBuilder ComponentReferenceCapture<T> (Action<T> action,
				[CallerLineNumber] int line = 0)
			=> Ref (action, line);
		...
		/// <param name="action">The action to be invoked whenever the referenced value changes.</param>
		/// <param name="line">The source code line number used to generate the sequence number.</param>
		public FluentRenderTreeBuilder Ref<T> (Action<T> action,
				[CallerLineNumber] int line = 0)
		{
			_builder.AddComponentReferenceCapture (GetSequence (line), value => action ((T) value));
			return this;
		}
		...
		{
			if (helper == null)
				throw new ArgumentNullException (nameof (helper), "A helper callback must be specified");
